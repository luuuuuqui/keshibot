...
async def handle(ctx: CommandContext) -> None:
    ...
    if not ctx.args:
        raise InvalidParameterError("Por favor, marque um usuario para promover.")
    user_lid = f"{only_numbers(ctx.args[0])}@lid"
    await ctx.bridge.group_participants_update(ctx.remote_jid, [user_lid], "promote")
    await ctx.send_success_reply("Usuario promovido com sucesso!")


command = Command(
    name="promover",
    description="Promove um usuario a administrador.",
